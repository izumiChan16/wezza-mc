...
| 服务 | 用途 | 是否长期运行 | 主机端口 |
|---|---|---|---|
| `minecraft` | 正式 Fabric 服务器 | 只在游玩时运行 | `127.0.0.1:25565` |
...
`mod publish` 只负责整合包发布。脚本、Compose、README、管理员手册或页面样式应单独提交：
...
只修改 README 或本管理员手册不会重新部署 Pages。
