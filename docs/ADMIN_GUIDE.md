...
1. **正式世界在 `runtime/data/`。** 不要手工清空、覆盖或与测试服目录混用。`./mcctl start` 只会创建停服快照并启动服务，绝不会解压或恢复任何备份。
...
| `backup-local` | 一次性本机在线备份；由 mcctl 按本次开关调度 | 按需临时运行 | 无 |
...
| `backup-remote` | restic/S3 增量备份、验证和清理 | 按需临时运行 | 无 |
...
| `ENABLE_REMOTE_BACKUP` | `false` | S3 配置完成；允许开服选择 S3 自动备份并启用关服上传 | 下次开服/关服 |
| `RESTIC_REPOSITORY` | 示例值 | S3-compatible restic 仓库 | 异地备份时 |
| `RESTIC_HOSTNAME` | `wezza-home-pc` | restic 快照来源标识 | 异地备份时 |
| `AWS_DEFAULT_REGION` | `us-east-1` | S3 区域；兼容旧配置 `auto` 的请求签名使用 `us-east-1` | S3 操作时 |
| `S3_ONLINE_LIMIT_KIB` | `5120` | 在线上传、下载和清理的每方向限速，KiB/s | 下次 S3 操作 |
| `S3_CAPACITY_BYTES` | `100000000000` | 桶容量，十进制字节 | 下次 S3 操作 |
| `S3_TARGET_BYTES` | `80000000000` | 容量清理目标 | 下次 S3 操作 |
| `S3_HIGH_WATER_BYTES` | `90000000000` | 暂停新增上传的警戒线 | 下次 S3 操作 |
...
`start` 会分别询问本次是否开启两小时本机备份（默认关闭）和两小时 S3 备份（默认开启）。选择只对本次运行有效；非交互脚本使用相同默认值，也可显式指定：

```bash
./mcctl start --local-auto=off --s3-auto=on
# S3 尚未配置时，必须明确关闭本次 S3 自动备份：
./mcctl start --local-auto=off --s3-auto=off
```

启用 S3 定时备份前先验证连接和仓库密码；失败不会偷偷退回无 S3 备份启动。正式服健康后启动独立后台调度进程，首次任务在两小时后执行，之后每次任务结束再等待两小时，不补跑积压任务。退出终端不影响调度，WSL/Docker 关闭则无法继续备份；重新开服会建立新会话。

`start` 会在正式数据非空时先创建停服状态快照，然后**强制重建**正式服容器。强制重建可确保 Docker Desktop/WSL 不会复用指向旧目录的 bind mount；它不会删除 `runtime/data/` 或任何备份，更不会恢复备份。服务器已运行时再次执行会直接拒绝，不会启动第二份。服务健康后，脚本会把当前 Packwiz 版本、清单哈希和 Git 提交记录到 `runtime/data/.mcctl-deployment`，供后续备份和恢复追溯。
...
1. 等待正在执行的备份结束，停用本次自动调度。
2. 在游戏内广播 10 秒提示，通过 RCON 执行 `save-all flush`。
3. 停止 frpc，正常停止 Minecraft（最多等待 120 秒），核实退出码为 0。
4. 对已经停止的服务器创建最终本机快照。
5. 如果启用了 S3，再不限速上传最终数据。

只有命令完成后，才关闭 Docker Desktop、Windows，或让电脑睡眠。正式服已退出时，`stop` 会停止残留 frpc 和旧本机调度容器，不会自动重复上传。无法正常保存或停止时会报错，不将其当作成功的关服恢复点。

关服后的本机或 S3 备份失败时，服务器保持停止。S3 失败不会删除已经完成的本机快照，执行 `./mcctl backup remote` 重试；本机快照失败则执行 `./mcctl backup` 重试。需要从一开始跳过本次关服上传时：
...
重启先询问本次两个定时开关并验证 S3，再完整关服和重新启动；支持与 `start` 相同的 `--local-auto`、`--s3-auto` 参数。关服备份失败时停止流程，不重新开服。重启仍可能生成关服快照和开服前快照，维护窗口需预留备份时间。
...
输出包括正式服务和 staging 状态、`WORLD_NAME` 对应的世界路径、世界目录是否存在、大小、最近修改时间、下次生成种子配置以及旧世界归档数量。它还会显示最近一次健康启动写入的部署记录，以及（如果发生过）最近一次正式恢复的归档身份与旧数据目录。已有世界的真实种子不会从 `level.dat` 中猜测；`Next generation seed` 只表示下次创建世界时 Compose 会使用的配置。
...
| 类型 | 位置 | 触发方式 |
|---|---|---|
| 在线备份 | `runtime/backups/local/` | 本次启用时每两小时、`backup`、测试前 |
| 停服快照 | `runtime/backups/offline/` | 开服前、正常关服后、停服时执行 `backup`、测试前 |

本机策略保留约 14 天，并限制最近约 20 份。具体在线文件名和格式由备份容器生成；脚本同时识别 `.tar.zst`、`.tgz` 和 `.tar.gz`。

关闭定时本机备份不会取消开服前和关服后的本机快照。手动创建本机备份只执行一次，不会开启自动调度。本机归档失败时会将本次文件隔离为 `.failed`，不会作为可恢复归档列出；排查后可手工清理这些失败文件。S3 在线备份直接增量读取数据目录，不会额外生成本机完整归档。
...
恢复和删除命令只接受列表显示的归档文件名，不接受任意路径。交互菜单的“删除备份（可多选）”支持批量选择：方向键菜单使用 Tab 勾选、Ctrl-A 全选、Ctrl-D 清空，再按 Enter；编号菜单输入 `1 3 5` 或 `1,3,5`，也可输入 `a` 全选。确认前列出所选文件、数量和总大小，使用一次 `[y/N]` 确认，默认不删除。

删除完成或取消确认后会刷新并留在备份选择列表，方便继续清理；按 q（方向键菜单按 Esc）才返回上级，列表为空时自动返回。直接使用以下非交互命令时仍必须显式传入 `--confirm`：
...
```bash
./mcctl backup delete offline/<文件名1> offline/<文件名2> --confirm
```
...
删除是永久操作，无法通过 mcctl 恢复。如果 local 和 offline 中存在同名文件，使用 `local/<文件名>` 或 `offline/<文件名>` 指定来源；备份、关服、开服、恢复、世界重置和测试会话使用同一操作锁；删除会等待当前操作结束。旧版本的 `backup-local` 常驻容器仍运行时禁止删除 local 归档。整批先检查文件是否存在、是否为符号链接及调度器限制，任一检查失败则整批不删除；执行中遇到文件系统错误则停止，并报告已经删除的数量。重复选择同一文件只删除一次。该命令不会影响远端 restic 备份。
...
不会立即删除。脚本会先验证归档可读取；如果解压过程失败，会尝试把原目录放回。恢复成功后，脚本会删除**已停止**的 `minecraft` 和 `backup-local` 容器，防止它们保留指向改名前目录的 bind mount；下一次 `./mcctl start` 会创建新容器。

每次成功恢复都会在以下位置新增不可执行的文本审计记录：

```text
runtime/restore-history/restore-<时间戳>.txt
```

记录包含归档 ID、SHA-256、大小、恢复时间、恢复前目录，以及备份中已有的部署版本/清单哈希。可通过 `./mcctl world status` 查看最近一条摘要。
...
文件管理网页不是 S3 API。AList/OpenList 通常在 `5244` 提供网页，需要在其服务配置中启用 S3，并在管理页面把桶映射到存储目录。例如：

```json
"s3": {"enable": true, "port": 5246, "ssl": false}
```

重启文件服务后，确认 S3 端口仅在需要的内网可达，并映射 `WezzaMC` 桶。参考 [OpenList S3 配置](https://doc.oplist.org/guide/advanced/s3)。项目不会自行改动路由器、端口映射或文件服务配置。

`.env` 示例（实际地址以文件服务设置为准）：

```text
ENABLE_REMOTE_BACKUP=false
RESTIC_REPOSITORY=s3:http://192.168.0.1:5246/WezzaMC/wezza-mc
RESTIC_HOSTNAME=wezza-home-pc
AWS_DEFAULT_REGION=us-east-1
S3_ONLINE_LIMIT_KIB=5120
S3_CAPACITY_BYTES=100000000000
S3_TARGET_BYTES=80000000000
S3_HIGH_WATER_BYTES=90000000000
```

使用路径式桶访问，保留桶名大小写；`wezza-mc/` 必须是本项目专用仓库前缀。凭据需要读取整个桶的对象清单，以便统计容量，以及仓库前缀内的读、写、删除权限。
...
密钥文件权限设为 `600`。保留现有 `secrets/restic_password.txt`，不要重新生成；丢失这个加密密码就无法恢复远端快照，需另存到可信密码管理器。

在 Minecraft 停止时执行：

```bash
./mcctl backup remote-init
```

对空仓库，该命令先在唯一的临时子前缀中验证上传、增量备份、快照清理、完整性检查和恢复比对，然后删除自己的测试对象，初始化正式仓库，最后把 `.env` 中 `ENABLE_REMOTE_BACKUP` 改为 `true`。测试不使用正式世界。有数据的仓库只验证现有仓库访问，绝不覆盖初始化；错误密码、网页响应或连接失败均保持失败状态。

现有仓库检查及重试：

```bash
./mcctl backup remote-check
./mcctl backup remote
./mcctl backup status
```

管理面板“世界与备份”也提供这些操作。`remote-check` 检查的是连接、容量读取和解密访问；成功检查不等于最近备份成功，两者分别显示。隔离验证失败的残留测试前缀会计入桶占用，应查清失败原因后清理该次测试前缀；不要删除正式仓库对象。

### 15.2 调度与限速

本次两小时 S3 开关与本机开关独立，默认 S3 开、本机关。在线上传、下载及清理的每方向上限为 5120 KiB/s（5 MiB/s，约 42 Mbps）；局域网千兆链路上仍需通过实际游玩观察延迟，必要时修改限速。

本机、S3、手动及关服备份串行执行。在线 S3 备份前执行 `save-off`、`save-all flush`，完成或失败后恢复 `save-on`；限速意味着大规模首次在线备份可能长时间关闭自动保存，建议首次完整备份在停服时运行。进程被强杀或系统掉电无法执行清理，重新进入运行中的服务器后可用 `./mcctl rcon save-on` 恢复保存。

正常关服后 S3 上传和清理不限速。上传期间开服、重启、正式恢复和世界重置会等待操作锁，避免读取中的数据改变。所有正式数据操作应通过 mcctl 执行；直接运行 Docker 或手工编辑数据不会遵守这把锁。

调度状态及日志位于忽略的 `runtime/backup-control/`。单次失败会记录并等待下一个两小时周期，不会高速重试。`backup remote` 可在线限速重试，也可停服后不限速重试；失败不会抹去以前的成功快照。S3 上传先标记为 pending，仅成功完成后才标记为正式快照；下一次备份会清理遗留的 pending 半成品。

### 15.3 100 GB 容量与保留策略

默认按十进制 100 GB 计算，80 GB 为清理目标，90 GB 为新增上传警戒线，预留空间用于新增数据、索引及重打包。统计使用整个桶分页列出的对象大小，不使用快照的逻辑文件大小；其他前缀计入占用但绝不自动删除。

通常保留最近 12 个快照、14 个日快照、8 个周快照和 12 个月快照。超过清理目标时，先回收中断上传留下的无引用数据，再按最旧优先删除本项目快照。最新成功快照及最近一次成功关服快照始终受保护，容量压力优先于日/周/月保留数量。

删除快照必须由 restic `forget` 和 `prune` 完成；每轮重打包最多 1 GB，清理后执行完整性结构检查。清理失败或容量仍达到警戒线时停止新增上传，不删除受保护快照。

上传期间定期检查容量，达到警戒线或无法读取容量时停止任务；清理操作使用更高的保护线，为释放空间留下操作余量。检测有时间间隔，不能作为硬配额，存储服务仍须实施 100 GB 限制。对象统计针对当前可列举对象；本方案假设桶不启用版本保留或隐藏回收站配额，否则存储端实际用量可能更高。
...
| `./mcctl start [--local-auto=on\|off] [--s3-auto=on\|off]` | 先做停服快照，再启动正式服和备份调度器 |
| `./mcctl stop` | 正常保存关服，再做本机快照及不限速 S3 上传 |
| `./mcctl stop --skip-remote` | 保留本机备份，但跳过本次远端备份 |
| `./mcctl restart [--local-auto=on\|off] [--s3-auto=on\|off]` | 完整安全关服后启动 |
...
| 命令 | 作用 |
|---|---|
| `./mcctl backup` | 创建一次在线本机备份或停服快照 |
| `./mcctl backup remote-init` | 停服时验证 S3、初始化空仓库并启用配置 |
| `./mcctl backup remote-check` | 检查 S3 清单读取和仓库访问 |
| `./mcctl backup remote` | 在线限速或停服后不限速备份，也用于失败重试 |
| `./mcctl backup status` | 查看两个定时开关及最近结果 |
| `./mcctl backup-list` | 列出备份、类型、单个大小及 local/offline/总计占用 |
| `./mcctl backup delete <文件名>... --confirm` | 删除一个或多个本机备份 |
