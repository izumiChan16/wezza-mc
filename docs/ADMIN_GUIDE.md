...
交互终端中无参数调用会打开中文状态总览。首页汇总本机配置、Docker、正式服和测试服状态、监听地址、当前整合包版本、候选变更是否经过测试接受、最新备份、本机备份总占用以及远端备份设置，并按当前状态给出建议操作。

默认使用 `fzf` 菜单：方向键移动，输入文字搜索，`Enter` 选择，`Esc` 返回。查询、日志、控制台、玩家与模组管理、备份和诊断操作完成后留在当前分类；开关或重启正式服、测试服状态变化、发布、世界重置和正式恢复成功后返回总览并刷新全局状态。若操作失败则留在当前分类，便于检查后重试。若 `fzf` 缺失会自动降级为编号菜单；也可以显式运行：
...
- **正式服务器**：启动、安全停止、重启、状态、日志、控制台和 RCON。
- **玩家与权限**：在线玩家、白名单、OP 和撤销 OP。
- **模组与客户端包**：添加、选择更新、安装侧、移除、校验和构建 `.mrpack`。
- **测试服与发布**：建立测试副本、查看日志、接受清单哈希以及发布小更新或完整更新。
- **世界与备份**：世界状态与重置、创建和选择备份、测试恢复、正式恢复和删除。
- **设置与诊断**：初始化、打开 `.env`、保存后校验、环境诊断和高级命令帮助。

普通状态变更使用 `y/N` 确认，大小写均可，直接按 Enter 默认为 N。交互菜单删除备份也使用这一确认方式；世界重置、正式恢复和发布仍要求输入包含世界名、备份 ID 或发布类型的完整确认短语。菜单不提供绕过测试接受的强制发布；确有必要时只能在理解风险后使用高级命令。
...
测试服会使用最近备份的世界副本。如果正式服正在运行，`stage start` 会先请求一次在线备份；如果正式服停止但已有数据，会先创建停服快照。
...
1. 刷新并验证当前候选 Packwiz 清单。
2. 创建或取得最新正式服备份。
3. 把旧测试数据移动为 `runtime/staging/data.previous.<时间戳>`。
4. 将最新备份恢复到新的 `runtime/staging/data/`；没有备份时创建空世界。
5. 删除旧的接受记录。
6. 启动本地 `pack-server` 和 `minecraft-staging`。
...
不要用 `./mcctl stage stop` 后再 `./mcctl stage start` 来验证同一测试世界；新的 `stage start` 会把现有 `runtime/staging/data/` 移走，并重新从最新正式备份建立副本。
...
脚本会记录当前整个 `pack/` 的 SHA-256 汇总值。它只证明“你接受的是这份清单”，不会自动判断游戏内容是否真的测试充分。
...
接受后如果修改任何 `pack/` 文件，包括模组元数据或再分发批准，接受记录就与当前清单不一致，发布会拒绝。此时必须重新启动、测试并接受。
...
`--force` 只跳过“必须存在匹配的 `stage accept` 记录”这一项，不会跳过：
...
`stage accept` 要求 `minecraft-staging` 正在运行且健康：
...
修复问题后重新启动测试。不要手工伪造 `ACCEPTED_PACK_SHA256`。
...
说明 `stage accept` 之后又修改了 `pack/`。重新测试当前清单，不要覆盖接受文件。
...
| 命令 | 作用 |
|---|---|
| `./mcctl stage start` | 从最新备份建立隔离测试服 |
