...
删除是永久操作，无法通过 mcctl 恢复。如果 local 和 offline 中存在同名文件，使用 `local/<文件名>` 或 `offline/<文件名>` 指定来源；在线备份调度器正在运行时不能删除 local 归档。整批先检查文件是否存在、是否为符号链接及调度器限制，任一检查失败则整批不删除；执行中遇到文件系统错误则停止，并报告已经删除的数量。重复选择同一文件只删除一次。该命令不会影响远端 restic 备份。
