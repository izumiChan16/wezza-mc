...
| 模拟距离 | 8 |
| 默认服务端内存 | 5 GB |
| 正式服主机端口 | `${MC_BIND_IP}:25565`，默认 `127.0.0.1` |
...
| `MC_BIND_IP` | `127.0.0.1` | 正式服主机绑定地址；局域网访问时设为 `0.0.0.0` |
| `MC_PORT` | `25565` | 正式服主机端口 |
...
修改绑定地址或端口后需要重新创建正式服容器。`MC_BIND_IP=0.0.0.0` 时，局域网玩家使用宿主机的局域网 IP 和 `MC_PORT` 连接；不要把 `0.0.0.0` 填进客户端。Windows 防火墙还必须允许该 TCP 端口。修改内存、MOTD 或人数后需要安全重启服务器才会生效。
